...
// if (!function_exists('dump_r')) {
    // modifier le nom de la fonction originelle
if (!function_exists('dump')) {
    function dump($raw, $ret = false, $html = true, $depth = 1e3, $expand = 1e3)
    {
...
// ajout d'une seconde fonction dd()
if (!function_exists('dd')) {
function dd($raw, $ret = false, $html = true, $depth = 1e3, $expand = 1e3)
{
// get the input arg passed to the function
$src = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
$idx = strpos($src[0]['file'], 'dump_r.php') ? 1 : 0;
...
do {
$line = $file[$src->line - $i++];
} while (strpos($line, 'dd') === false);

preg_match('/dd\((.+?)\)?(?:$|;|\?>)/', $line, $m);
$key = $m[1];

$key = trim(explode(',', $key)[0]);

Core::dump_r($src->file, $src->line, $key, $raw, $ret, $html, $depth, $expand);
die();
}
}
...
elseif (is_callable($raw) && !(is_object($raw) && !($raw instanceof \Closure)) && !(is_string($raw) &&
function_exists($raw))) {
...
$type->class[] = '_Binary';
} elseif (strlen($raw) > 5 && preg_match('#[:/-]#', $raw) && ($ts = strtotime($raw)) !== false) {
...
// _JSON
elseif ($raw[0] == '{' && $json = json_decode($raw)) {
